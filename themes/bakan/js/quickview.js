	...
	**/
	if( $('body').html().match( /sw-quickview-bottom/ ) ){
		...
		$(document).on( 'click', 'a[data-type="quickview"]', function(){
			var product_id = $(this).data( 'product_id' ), ajaxurl = quickview_param.ajax_url.replace( '%%endpoint%%', 'bakan_quickviewproduct' );
			ajaxurl = ajaxurl.replace( '%endpoint%', 'bakan_quickviewproduct' );
			qv_target.addClass( 'show loading' );
			if( product_id > 0 ){
				qv_target.addClass( 'quickview-video' );
			}
			var data 		= {
				action: 'bakan_quickviewproduct',
		...
		$( '.quickview-close' ).on('click', function(){
			...
			qv_target.removeClass( 'quickview-video' );
