...
** Maintaince Mode
*/
function bakan_template_load( $template ){ 
	if( !is_user_logged_in() && swg_options('maintaince_enable') ){
		$template = SWPATH . 'inc/maintaince/maintaince.php';
	}
	return $template;
}
add_filter( 'template_include', 'bakan_template_load' );
...
function sw_maintaince_script(){
	$output = '';
	$countdown = swg_options('maintaince_date');
	if( $countdown != '' ):
		$output .= 'jQuery(function($){
		"use strict";
		function bakan_check_height(){
		...
		$(window).on( "load", function(){
			...
			$(document).ready(function(){ 
				...
				$("#countdown-container").ClassyCountdown({
					...
					labelsOptions: {
						lang: {
						days: "'. esc_html__( 'Days', 'sw_core' ) .'",
						hours: "'. esc_html__( 'Hours', 'sw_core' ) .'",
						minutes: "'. esc_html__( 'Mins', 'sw_core' ) .'",
						seconds: "'. esc_html__( 'Secs', 'sw_core' ) .'"
						},
