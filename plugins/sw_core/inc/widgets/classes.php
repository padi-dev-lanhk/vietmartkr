...
if ( !class_exists('SW_Widget') ):

abstract class SW_Widget extends WP_Widget{
	...
	protected function init(){
		if( current_theme_supports( 'sw_theme' ) ){
			...
			$this->base_tpl_path = apply_filters('gaion_widget_template_base', $this->base_path.'/widgets/'.$this->id_base.'/tmpl');
			$this->override_tpl_path = apply_filters('gaion_widget_override_base', get_template_directory().'/widgets/'.$this->id_base);
		}else{
	...
	public function gaion_trim_words( $text, $num_words = 30, $more = null ) {
	...
	public function widget_template_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( array_key_exists('ov', $templates) && count($templates['ov']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Override by Theme', 'sw_core') .'">';
		...
		if ( array_key_exists('df', $templates) && count($templates['df']) ){
			$select_html .= '<optgroup label="'. esc_attr__('Default Template', 'sw_core') .'">';
	...
	public function category_select( $field_name, $opts = array(), $field_value = null ){
		...
		if ( (is_string($opts['multiple']) && strtolower($opts['multiple'])=='multiple') || (is_bool($opts['multiple']) && $opts['multiple']) ){
			...
			if (array_key_exists('allow_select_all', $opts) && $opts['allow_select_all']){
				...
				$allow_select_all = '<option value="0">'. esc_html_e( 'All Categories', 'sw_core' ) .'</option>';
			}
		}
	...
	public function form( $instance ){
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('title') ); ?>"><?php esc_html_e('Title', 'sw_core')?></label>
			<br />
			<input class="widefat"
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('widget_template') ); ?>"><?php esc_html_e("Template", 'sw_core') ?></label>
			<br/>
