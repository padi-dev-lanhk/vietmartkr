...
final class Plugin
{
    ...
    function __construct(array $settings = [])
    {
        ...
        // Define constants.
    	define('SWWE_PLUGIN_URL', __DIR__ . '/');
    	define('SWWE_PLUGIN_URI', str_replace(['http:', 'https:'], '', plugins_url('/', __FILE__)));

        // Bind important events.
    	add_action('plugins_loaded', [$this, '_install'], 10, 0);
    	add_action('activate_swe/sw-woo-elements.php', [$this, '_activate']);
    	add_action('deactivate_swe/sw-woo-elements.php', [$this, '_deactivate']);
    }
    ...
    function _install() {
    	...
    	if(!did_action('elementor/loaded')) {
    		add_action('admin_notices', function() {
    			...
    			if ($this->is_plugin_installed($elementor)) {
    				...
    				$message = sprintf(__('%1$sSw Elements%2$s requires %1$sElementor%2$s plugin to be active. Please activate Elementor to continue.', 'sw-woo-elements'), "<b>", "</b>");

    				$button_text = __('Activate Elementor', 'sw-woo-elements');
    			} else {
    				$activation_url = wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=elementor'), 'install-plugin_elementor');

    				$message = sprintf(__('%1$sSw Elements%2$s requires %1$sElementor%2$s plugin to be installed and activated. Please install Elementor to continue.', 'sw-woo-elements'), '<b>', '</b>');
    				$button_text = __('Install Elementor', 'sw-woo-elements');
    			}
        ...
        load_textdomain( 'sw-woo-elements', SWWE_PLUGIN_URL.'languages/sw-woo-elements-'.determine_locale().'.mo' );
        load_plugin_textdomain('sw-woo-elements', false, SWWE_PLUGIN_URL.'languages');

        require SWWE_PLUGIN_URL . 'lib/class-swe-functions.php';
        require SWWE_PLUGIN_URL . 'lib/class-swe-elements-category.php';
        require SWWE_PLUGIN_URL . 'lib/class-swe-widgets-manager.php';
        require SWWE_PLUGIN_URL . 'lib/class-swe-assets-manager.php';
    }
