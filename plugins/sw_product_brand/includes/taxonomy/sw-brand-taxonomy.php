...
class sw_brand_taxonomy {
	...
	*/
	...
	?>
			...
			<div class="form-field">
				<label><?php _e( 'Thumbnai 2', 'sw_product_brand' ); ?></label>
				<div id="product_brand_thumbnail1" style="float: left; margin-right: 10px;"><img src="<?php echo esc_url( wc_placeholder_img_src() ); ?>" width="60px" height="60px" /></div>
				<div style="line-height: 60px;">
					<input type="hidden" id="product_brand_thumbnail_id1" name="product_brand_thumbnail_id1" />
					<button type="button" class="upload_image_button1 button"><?php _e( 'Upload/Add image', 'sw_product_brand' ); ?></button>
					<button type="button" class="remove_image_button1 button"><?php _e( 'Remove image', 'sw_product_brand' ); ?></button>
				</div>
				<script type="text/javascript">

					// Only show the "remove image" button when needed
					if ( ! jQuery( '#product_brand_thumbnail_id1' ).val() ) {
					...
					jQuery( document ).on( 'click', '.upload_image_button1', function( event ) {
						...
						file_frame.on( 'select', function() {
							...
							jQuery( '#product_brand_thumbnail_id1' ).val( attachment.id );
							jQuery( '#product_brand_thumbnail1 > img' ).attr( 'src', attachment.sizes.thumbnail.url );
					...
					jQuery( document ).on( 'click', '.remove_image_button1', function() {
						jQuery( '#product_brand_thumbnail1 img' ).attr( 'src', '<?php echo esc_js( wc_placeholder_img_src() ); ?>' );
						jQuery( '#product_brand_thumbnail_id1' ).val( '' );
						jQuery( '.remove_image_button1' ).hide();
						return false;
					});
		...
		public function edit_category_fields( $term ) {
			...
			$thumbnail_id1 	= absint( get_term_meta( $term->term_id, 'thumbnail_bid1', true ) );
			...
			if ( $thumbnail_id1 ) {
				$image1 = wp_get_attachment_thumb_url( $thumbnail_id1 );
			} else {
			...
			<tr class="form-field">
				<th scope="row" valign="top"><label><?php _e( 'Thumbnail 2', 'sw_product_brand' ); ?></label></th>
				<td>
					<div id="product_brand_thumbnail1" style="float: left; margin-right: 10px;"><img src="<?php echo esc_url( $image1 ); ?>" width="60px" height="60px" /></div>
					<div style="line-height: 60px;">
						<input type="hidden" id="product_brand_thumbnail_id1" name="product_brand_thumbnail_id1" value="<?php echo $thumbnail_id1; ?>" />
						<button type="button" class="upload_image_button1 button"><?php _e( 'Upload/Add image', 'sw_product_brand' ); ?></button>
						<button type="button" class="remove_image_button1 button"><?php _e( 'Remove image', 'sw_product_brand' ); ?></button>
					</div>
					<script type="text/javascript">
						...
						// Only show the "remove image" button when needed
						if ( '0' === jQuery( '#product_brand_thumbnail_id1' ).val() ) {
							jQuery( '.remove_image_button1' ).hide();
						}
		...
		public function save_category_fields( $term_id, $tt_id = '', $taxonomy = '' ) {
			...
			if ( isset( $_POST['product_brand_thumbnail_id1'] ) && 'product_brand' === $taxonomy ) {
				update_term_meta( $term_id, 'thumbnail_bid1', absint( $_POST['product_brand_thumbnail_id1'] ) );
			}
		}
