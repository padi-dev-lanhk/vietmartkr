...
class sw_brand_filter_widget extends WC_Widget {
	...
	function __construct(){
		...
		add_filter( 'woocommerce_widget_get_current_page_url', array( $this, 'sw_brand_filter_get_current_page' ) );
	}
	
	function sw_brand_filter_get_current_page( $link ){
		if( is_page() ){
	...
	public function widget( $args, $instance ) {
		...
		extract($instance);
		$terms = get_terms( 'product_brand', array( 'parent' => '', 'hide_empty' => 0, 'number' => $number ) );		
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of brand', 'sw_product_brand')?></label>
			<br />
