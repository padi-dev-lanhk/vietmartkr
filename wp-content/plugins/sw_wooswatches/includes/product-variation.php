			...
			if ( $product && taxonomy_exists( $attribute ) ) {
				...
				foreach ( $terms as $key => $term ) {
					...
					if ( in_array( $term->slug, $options ) ) {
						$html .= '<label class="radio-label sw-radio-variation sw-radio-variation-color '. ( ( $color != '' ) ? 'style1': "") .' sw-radio-variation-'. esc_attr( $key .' '. $active ) .'" title="'. esc_attr( $term->slug )  .'" for="'. esc_attr( $term->slug . '_' . $key ) . '" '. $parent_attr .'>';
				...
				foreach ( $options as $key => $option ) {					
					...
					$html .= '<label class="radio-label sw-radio-variation sw-radio-variation-image sw-radio-variation-'. esc_attr( $key .' '. $active ) .'"  title="'. esc_attr( $option )  .'"  for="'. esc_attr( $option . '_' . $key ) . '" '. $parent_attr .'>';
