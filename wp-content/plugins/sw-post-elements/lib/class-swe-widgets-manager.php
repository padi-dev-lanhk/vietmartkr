...
final class SWE_Widgets_Manager {
    ...
    function _registerWidgets(Widgets_Manager $widget_manager) {
        if (!class_exists('SWE\SWE_Widget_Base')) {
            require SWPE_PLUGIN_URL . 'lib/class-swe-widget-base.php';
        }

        $filesPlugin = new DirectoryIterator(SWPE_PLUGIN_URL . 'src/widgets');

        foreach ($filesPlugin as $file) {
            $filename = $file->getFilename();
            if (strpos($filename, '.php') !== false) {
                require_once SWPE_PLUGIN_URL . 'src/widgets/' . $filename;
            }
        }
    }
