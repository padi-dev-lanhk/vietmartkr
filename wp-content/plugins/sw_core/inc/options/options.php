...
if ( ! class_exists('SWG_Options') ){
	...
	class SWG_Options{
		public $dir = SWG_OPTIONS_DIR;
		public $url = SWG_OPTIONS_URL;
		public $page = '';
		public $args = array();
		...
		public function __construct($sections = array(), $args = array()){
			...
			$this->args = apply_filters( 'SWG_Options_args_' . $this->args['opt_name'], $this->args );
			...
			//get sections
			$this->sections = apply_filters( 'SWG_Options_sections_' . $this->args['opt_name'], $sections );				
		...
		function _enqueue(){
			...
			wp_enqueue_style(
				'sw-opts-css',
				SWG_OPTIONS_URL . '/options/css/options.css',
				array('farbtastic'),
				time(),
				'all'
			);
			...
			wp_enqueue_script(
				'sw-opts-js',
				SWG_OPTIONS_URL.'/options/js/options.js',
				array('jquery'),
				time(),
				true
			);
				...
				if(isset($section['fields'])){
					...
					foreach($section['fields'] as $fieldk => $field){
						...
						if(isset($field['validate'])){
							$validate = 'SWG_Validation_'.$field['validate'];
							
							if(!class_exists($validate)){
								//include_once (SWG_OPTIONS_URL.'/options/validation/'.$field['validate'].'/validation_'.$field['validate'].'.php');
							}//if
		...
		public function getFieldInstance( $field = array() ){
			...
			if ( !class_exists($classname) ){
				$classfile = SWG_OPTIONS_DIR."/options/fields/$type/field_$type.php";
		...
		public function getCpanelField( $field ){
			...
			$classname = 'SWG_Options_'.$field['type'];
			if ( !class_exists($classname) ){
				$classfile = SWG_OPTIONS_DIR.'/options/fields/'.$field['type'].'/field_'.$field['type'].'.php';
				if ( file_exists($classfile) ){
					include $classfile;
				}
			}
		...
		function _field_input($field){
			...
			if ( isset( $field['type'] ) && $this->prepare_field( $field['type'] ) ){
				$field_class = 'SWG_Options_'.$field['type'];
